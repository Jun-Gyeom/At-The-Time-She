...
public class DialogueManager : Singleton<DialogueManager>
{
    ...
    [HideInInspector] public Image verandaCharacterPortraitImage;             // 캐릭터 초상화 이미지
    [HideInInspector] public TMP_Text verandaDialogueText;                    // 대화 텍스트
    // 베란다 대화 GUI - 독백
    [HideInInspector] public GameObject verandaMonologuePanelGameObject;      // 독백 패널 오브젝트
    [HideInInspector] public TMP_Text verandaMonologueText;                   // 독백 텍스트
    ...
    private SceneType _sceneType;                 // 대화 씬 종류
    ...
    public void StartDialogue(int sceneID)
    {
        ...
        _sceneType = dialogueScene.sceneType;           // 대화 씬 종류 설정
    ...
    public void NextDialogue(int dialogueID = -1)
    {
        ...
        {
            illustrationController.ShowIllustration(dialogueElement.illustrationName);
        }
        ...
        {
            // 선택지 출력 로직 
            choiceController.ShowChoice(_choiceID, this);
            Debug.Log("선택지 출력!");
        ...
        {
            // 대화 화면 출력 처리
            _hasCharacterPortrait = HasCharacterPortrait(dialogue);
    ...
    {
        return dialogueText.Replace("(주인공)", "플레이어 이름(추후 변경)").Replace("Ć", ","); // 주인공 -> 플레이어 이름, Ć -> ,
    }
    ...
    private void EndDialogue()
    {
        ...
        // 일러스트 숨기기
        illustrationController.HideIllustration();
        // 표시 아이템 숨기기
        displayItemController.HideDisplayItem();
        // 대화 창 숨기기
    ...
    private IDialogueUI SelectDialogueUI()
    {
        ...
        switch (_sceneType) 
        {
            case SceneType.General when _hasCharacterPortrait: // 현재 대화의 초상화 존재 여부
                // 대화
            ...
            case SceneType.General:
                // 독백
                dialogueUI = new MonologueUI();
                    
                break;
            case SceneType.Ending:
                // 엔딩
                dialogueUI = new EndingUI();
                    
                break;
            case SceneType.True_Ending:
                // 진엔딩
                dialogueUI = new TrueEndingUI();
    ...
    private bool HasCharacterPortrait(Dialogue dialogue)
    {
        return dialogue.characterPortrait != "";
    }
